...
public class PlayState extends State {
	...
	static int marginSize = cellSize * 3, xOffset = 22, yOffset = 17;
	...
	public PlayState(Connect4Pannel pannel, MouseHandler m) {
		...
		playerYellow = new Connect4AI(logic, false);// new
													// Connect4HumanPlayer(logic,
													// false, mouse, this);//
	...
	public void drawFallingChip(Graphics g) {
		...
		g.drawImage(getChipImage(logic.getSpace(move.x, move.y) == 1), 5 + move.x * (cellSize + cellSpacing) + xOffset,
				fallingChipY, null);// (x, y, width, height);
		// g.fillRect(5+ move.x * (cellSize + cellSpacing) + xOffset,
		// fallingChipY, 60, 60);
			...
			for (int y = 0; y < logic.getSizeY(); y++) {
				if (x != lastMove.x || y != lastMove.y) {
					if (logic.getSpace(x, y) == Connect4Logic.RED_CHIP_INDEX) {
						...
						g.drawImage(redChip, 5 + x * (cellSize + cellSpacing) + xOffset,
								y * (cellSize + cellSpacing) + yOffset, null);
						// g.fillOval( 5+ x * (cellSize + cellSpacing), y *
						// (cellSize + cellSpacing), cellSize, cellSize);
					}
					if (logic.getSpace(x, y) == Connect4Logic.YELLOW_CHIP_INDEX) {
						g.setColor(Color.YELLOW);
						g.drawImage(yellowChip, 5 + x * (cellSize + cellSpacing) + xOffset,
								y * (cellSize + cellSpacing) + yOffset, null);// (x,
																				// y,
																				// width,
																				// height);
						// g.fillOval(5+ x * (cellSize + cellSpacing), y *
				...
				// for (int x = 0; x < logic.getSizeX(); x++) {
				// for (int y = 0; y < logic.getSizeY(); y++) {
				g.drawImage(boardImage, 5 + x * (cellSize + cellSpacing) + xOffset - cellSpacing / 2,
						y * (cellSize + cellSpacing) + yOffset - cellSpacing / 2, cellSize + cellSpacing,
						cellSize + cellSpacing, null);
				...
				g2.drawOval(5 + x * (cellSize + cellSpacing) + xOffset, y * (cellSize + cellSpacing) + yOffset,
						cellSize, cellSize);
	...
	public void drawCircle(Point p, Graphics g) {
		g.drawOval(5 + p.x * (cellSize + cellSpacing) + xOffset, p.y * (cellSize + cellSpacing) + yOffset, cellSize,
				cellSize);
	}
	...
	public void update() {
		...
		if (isColorPickerOpen) {
			...
			// logic.makeMove(Connect4AI.selectMove(logic.board),
			// logic.isRedTurn);
			// }
	...
	Button ChooseaColor = new Button(colorPickerPosition.x + 10, colorPickerPosition.y + 10, 185, 90, " Pick a Color ",
			Color.blue, Color.yellow, Color.yellow, Color.yellow, Color.yellow, Color.yellow);

	Button RedButton = new Button(colorPickerPosition.x + 10, colorPickerPosition.y + 110, 85, 35, "Red",
			Color.lightGray, Color.red, Color.black, Color.red, Color.gray, Color.yellow);
	Button YellowButton = new Button(colorPickerPosition.x + 110, colorPickerPosition.y + 110, 85, 35, "Yellow",
			Color.lightGray, Color.yellow, Color.black, Color.yellow, Color.gray, Color.red);
	...
	void drawColorPicker(Graphics g) {
		...
		// g.drawString(" Pick a Color ", colorPickerPosition.x + 50,
		// colorPickerPosition.y + 60);
		...
		// menuButton.draw(g,
		// menuButton.isMouseOver(pannel.getMousePosition()));

		g.setColor(Color.red);
		// g.drawRect(colorPickerPosition.x, colorPickerPosition.y,
		// colorPickerSize.x, colorPickerSize.y);
